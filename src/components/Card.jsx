...
const Card = ({ book, index }) => {
  ...
  const reservation = () => {
    if (user == null) {
      toast("Bạn hãy đăng nhập để đăng ký mượn");
    } else {
      createReservation({ userId: user.id, isbn: book.isbn })
        .then(() => {
          alert("thành công");
        })
        .catch((error) => {
  ...
  const actionLikeBook = () => {
    likeBook(book.isbn)
      ...
      .catch(() => {
        toast("Bạn hãy đăng nhập để thích sách");
      });
  };
  // const likesBook = () => {
      ...
      return (
        <div>
              ...
              <div className="flex  text-sm gap-x-7">
                <div className="flex-shrink-0">
                  <img
                    width={100}
                    height={140}
                    src={`${process.env.REACT_APP_API_URL}/books/image/${book.isbn}`}
                    alt=""
                  />
                </div>
                <div className="flex flex-col gap-y-5 gap-x-2  ">
                  <div className="grid grid-cols-6  gap-x-5">
                    <div className="w-24">Tiêu đề</div>
                    <div className="col-span-5">{book.title}</div>
                  </div>
                  <div className="grid grid-cols-6  gap-x-5">
                    <div className="w-24">Tác giả</div>
                    <div className="col-span-5">{book.authors}</div>
                  </div>
                  <div className="grid grid-cols-6  gap-x-5">
                    <div className="w-24">Dạng tài liệu</div>
                    <div className="col-span-5">{book.category}</div>
                  </div>
                  <div className="grid grid-cols-6 gap-x-5">
                    <div className="w-24">Mô tả</div>
                    <div className="col-span-5">{book.content}</div>
                  </div>
                  <div className="grid grid-cols-6  gap-x-5">
                    <div className="w-24">Isbn</div>
                    <div className="col-span-5">{book.isbn}</div>
                  </div>
                  <div className="grid grid-cols-6  gap-x-5">
                    <div className="w-24">Giá</div>
                    <div className="col-span-5">{book.price}</div>
                  </div>
                  <div className="grid grid-cols-6  gap-x-5">
                    <div className="w-24">Nhà xuất bản</div>
                    <div className="col-span-5">{book.publisher}</div>
                  </div>
                  <div className="grid grid-cols-6  gap-x-5">
                    <div className="w-24">Có sẵn</div>
                    <div className="col-span-5">{book.ready}</div>
                  </div>
                  <div className="grid grid-cols-6  gap-x-5">
                    <div className="w-24">Tags</div>
                    <div className="col-span-5">{book.tags?.join(", ")}</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="flex items-center">
            <button
              className="mt-5 px-2 py-1 text-sm flex items-center gap-x-2 bg-blue-400"
              onClick={() => setDetail(!isDetail)}
            >
              {isDetail ? "Tổng quan " : "Chi tiết"}
              <ArrowRight size={15} />
            </button>
            {isDetail && (
              <div className="flex ml-auto gap-x-7 mt-3">
                <div
                  onClick={reservation}
                  className="px-2 py-1 cursor-pointer  flex justify-center border-2 bg-blue-600 rounded-lg font-serif "
                >
                  <Activity />
                  Đăng ký mượn
                </div>
    ...
    <div className="border-[1px] border-gray-400 px-4 py-2 rounded-md bg-gray-200 mb-2 mx-40">
      ...
      <div className="flex flex-col mt-3">
        <div className="flex gap-x-1 mb-4">
          <div
            className="py-1 px-2 text-sm rounded-sm text-white bg-gray-500 cursor-pointer hover:text-gray-300"
